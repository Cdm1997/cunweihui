...
/**
 * Servlet implementation class DistributeSearch_Servlet
 */
public class DistributeSearch_Servlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		for(int i=1; i <= pages; i++){
			...
			if(i == currPage){
				...
				// 构建分页条
				sb.append("<a href='DistributeSearch_Servlet?page=" + i + "'>" + i + "</a>");
			}
			// 构建分页条
			sb.append("　");
		}
		...
		request.getRequestDispatcher("distribute_list.jsp").forward(request, response);
	}
}