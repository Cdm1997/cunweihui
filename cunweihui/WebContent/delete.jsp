...
if (ttt==2) {
	String id=request.getParameter("id");
	String sqlo="delete from wuzi where id='"+id+"'";
	PreparedStatement ps = conn.prepareStatement(sqlo);
	ps.executeUpdate();
	out.println("<script lanage='javascript'>alert('删除成功！！');window.location.href='WuziServlet';</script>");
	}
if (ttt==3) {
	String id=request.getParameter("id");
	String sqlo="delete from wuzifenpei where id='"+id+"'";
	PreparedStatement ps = conn.prepareStatement(sqlo);
	ps.executeUpdate();
	out.println("<script lanage='javascript'>alert('删除成功！！');window.location.href='DistributeServlet';</script>");
	}
if (ttt==4) {
	String id=request.getParameter("id");
