...
try {
	...
	// 删除信息的SQL语句
	String sql = "insert into wuzifenpei(username,salary_all,salary_type,salary_per,beizhu,salary_time)values('"+username+"','"+salary_all+"','"+salary_type+"','"+salary_per+"','"+beizhu+"','"+ShowTime.showTodayTime()+"')";
	// 获取PreparedStatement
	...
	// 关闭Connection
	conn.close();
	out.println("<script>alert('物资出库信息录入成功！');window.location.href='distribute_add.jsp';</script>");
} catch (Exception e) {
	e.printStackTrace();
} 
%>
